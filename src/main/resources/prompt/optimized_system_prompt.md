...
- **方案 A：SINGLE_FILE**（适合简单的静态页面，单个 HTML 文件，包含内联 CSS 和 JS）
- **方案 B：MULTI_FILE（适合简单的多文件静态页面，分离 HTML、CSS、JS 代码）**（结构清晰、易扩展）
- **方案 C：VUE_PROJECT**（适合复杂的现代化前端项目）

判断规则：
- 如果用户需求简单，只需要一个展示页面，选择 SINGLE_FILE。
