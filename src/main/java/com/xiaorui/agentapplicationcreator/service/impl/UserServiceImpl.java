...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements UserService {
    ...
    public String userRegister(String userEmail, String loginPassword, String checkPassword, String emailVerifyCode) {
        if (StrUtil.hasBlank(userEmail, loginPassword, checkPassword, emailVerifyCode)) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "request params are blank");
        }
    ...
    public UserVO userLogin(String userEmail, String loginPassword, HttpServletRequest request) {
        ...
        if (Objects.equals(user.getUserStatus(), UserStatusEnum.BANNED.getValue())
            ...
            throw new BusinessException(ErrorCode.NOT_AUTH_ERROR, "user is banned");
        }
    ...
    public boolean sendEmailCode(String userEmail, String type, HttpServletRequest request) {
        ...
        if (StrUtil.isNotBlank(ipCount) && Integer.parseInt(ipCount) > IP_REQUEST_LIMIT) {
            throw new BusinessException(ErrorCode.TOO_MANY_REQUEST, "too many requests from current ip");
        }
        ...
        if (StrUtil.isNotBlank(emailCount) && Integer.parseInt(emailCount) > EMAIL_REQUEST_LIMIT) {
            throw new BusinessException(ErrorCode.TOO_MANY_REQUEST, "too many requests for current email");
        }
        ...
        try {
            ...
            throw new BusinessException(ErrorCode.SYSTEM_ERROR, "failed to send email code");
        }
    ...
    public Map<String, String> getPictureVerifyCode() {
        ...
        String captchaChallengeId = UUID.fastUUID().toString(true);
        stringRedisTemplate.opsForValue().set(CAPTCHA_CACHE_PREFIX + captchaChallengeId, captchaCode, 300, TimeUnit.SECONDS);
        Map<String, String> data = new HashMap<>();
        data.put("base64Captcha", base64Captcha);
        data.put("encryptedCaptcha", captchaChallengeId);
        return data;
    }

    @Override
    public boolean checkPictureVerifyCode(String verifyCode, String serverVerifyCode) {
        if (StrUtil.hasBlank(verifyCode, serverVerifyCode)) {
            log.info("captcha verify failed: params blank");
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "captcha is invalid");
        }
        String captchaCacheKey = CAPTCHA_CACHE_PREFIX + serverVerifyCode.trim();
        String actualCaptcha = stringRedisTemplate.opsForValue().get(captchaCacheKey);
        stringRedisTemplate.delete(captchaCacheKey);
        if (StrUtil.isBlank(actualCaptcha)) {
            log.info("captcha verify failed: challenge expired");
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "captcha expired");
        }
        if (!actualCaptcha.equalsIgnoreCase(verifyCode.trim())) {
            log.info("captcha verify failed: value mismatch");
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "captcha is invalid");
        }
        return true;
    }
    ...
    public QueryWrapper getQueryWrapper(UserQueryRequest userQueryRequest) {
        ThrowUtil.throwIf(userQueryRequest == null, ErrorCode.PARAMS_ERROR, "request params are blank");
    ...
    public boolean changeUserPassword(String oldPassword, String newPassword, String checkPassword) {
        ...
        tokenStoreManager.deleteAllToken(userId);
    ...
    public boolean resetUserPassword(String userEmail, String emailVerifyCode, String newPassword, String checkPassword) {
        ...
        tokenStoreManager.deleteAllToken(targetUser.getUserId());
    ...
    public String updateUserAvatar(MultipartFile multipartFile) {
        ...
        ThrowUtil.throwIf(multipartFile.isEmpty(), ErrorCode.PARAMS_ERROR, "file must not be empty");
        ...
        ThrowUtil.throwIf(!result, ErrorCode.OPERATION_ERROR, "failed to update avatar");
    ...
    public boolean banOrUnbanUser(String userId, Boolean isUnban, User admin) {
        ...
        if (admin == null || !UserConstant.ADMIN_ROLE.equals(admin.getUserRole())) {
            throw new BusinessException(ErrorCode.NOT_AUTH_ERROR, "admin role required");
        }
        User targetUser = this.getById(userId);
        if (targetUser == null) {
            throw new BusinessException(ErrorCode.NOT_FOUND_ERROR, "user not found");
        }
        ThrowUtil.throwIf(admin.getUserId().equals(userId), ErrorCode.OPERATION_ERROR, "cannot change current admin status");
        ThrowUtil.throwIf(UserConstant.ADMIN_ROLE.equals(targetUser.getUserRole()), ErrorCode.OPERATION_ERROR, "cannot ban admin user");
        ...
        ThrowUtil.throwIf(isUnban == !isBanned, ErrorCode.OPERATION_ERROR, isUnban ? "user is not banned" : "user is already banned");
    ...
    public boolean updateUserRole(String userId, String userRole, User admin) {
        ThrowUtil.throwIf(StrUtil.hasBlank(userId, userRole), ErrorCode.PARAMS_ERROR, "request params are blank");
        ThrowUtil.throwIf(admin == null || !UserConstant.ADMIN_ROLE.equals(admin.getUserRole()), ErrorCode.NOT_AUTH_ERROR, "admin role required");
        ThrowUtil.throwIf(admin.getUserId().equals(userId), ErrorCode.OPERATION_ERROR, "cannot change current admin role");
        ThrowUtil.throwIf(UserRoleEnum.getEnumByValue(userRole) == null, ErrorCode.PARAMS_ERROR, "invalid user role");
        User targetUser = this.getById(userId);
        ThrowUtil.throwIf(targetUser == null, ErrorCode.NOT_FOUND_ERROR, "user not found");
        ThrowUtil.throwIf(Objects.equals(targetUser.getUserStatus(), UserStatusEnum.BANNED.getValue()), ErrorCode.OPERATION_ERROR, "unban user before changing role");
