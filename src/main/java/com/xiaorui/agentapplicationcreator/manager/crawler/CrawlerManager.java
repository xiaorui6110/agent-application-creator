...
public class CrawlerManager {
        ...
        if (count > banCount) {
            if (loginUser != null) {
                ...
                if (loginUser.getUserRole().equals(UserConstant.ADMIN_ROLE)){
                    log.warn("警告：管理员访问频繁, userId={}, count={}", loginUser.getUserId(), count);
                    return;
                }
                // 对于普通用户，封号并踢下线
                ...
                throw new BusinessException(ErrorCode.NOT_AUTH_ERROR, "访问次数过多，已被封号");
            } else {
                // 对于未登录用户，封禁 IP
                banIp(identifier);
                throw new BusinessException(ErrorCode.NOT_AUTH_ERROR, "访问次数过多，IP 已被封禁");
            }
        } else if (count > warnCount) {
            // 记录警告日志
            if (loginUser != null) {
                log.warn("警告：用户访问频繁, userId={}, count={}", loginUser.getUserId(), count);
            } else {
    ...
    private void sendCrawlerWarningEmail(User user, String ipAddress, long accessCount) throws IOException {
        ClassPathResource resource = new ClassPathResource("static/html/crawler_warning.html");
        String template;
        ...
        String emailContent = template
                .replace(":userId", String.valueOf(user.getUserId()))
        ...
        // 发送邮件
        emailSenderUtil.sendWarningEmail(adminEmail, emailContent);
    }
